...
#[CoversClass(CredentialIssuerMetadataService::class)]
#[AllowMockObjectsWithoutExpectations]
class CredentialIssuerMetadataServiceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // What a wallet may sign its key proof with is a different list from what this issuer signs
        // credentials with: every algorithm this deployment can verify, deduplicated. A real bag, with a
        // repeat in it, so that the deduplication is what is asserted rather than assumed.
        $supportedAlgorithmsMock = $this->createMock(SupportedAlgorithms::class);
        $supportedAlgorithmsMock->method('getSignatureAlgorithmBag')->willReturn(new SignatureAlgorithmBag(
    ...
    protected function sut(): CredentialIssuerMetadataService
    {
        return new CredentialIssuerMetadataService(
    ...
    /**
     * The document as its two consumers receive it. Both publish it as JSON, so it is read back through
     * an encode and decode here the way a wallet reads it, rather than as the PHP array it is built as.
     *
     ...
     * @throws \JsonException
     */
    public function testDescribesItselfToWalletsFromTheCommonMetadata(): void
    {
        $metadata = $this->publishedMetadata();
    ...
    /**
     * A `vc+sd-jwt` configuration is published with the JSON-LD context its credentials will carry,
     * which is the resolver's answer rather than whatever the operator wrote. It goes where the data
     * model puts it: inside `credential_definition` when the configuration states one, and at the top
     * of the configuration otherwise.
     *
     * @throws \SimpleSAML\Module\oidc\Server\Exceptions\OidcServerException
     * @throws \JsonException
     */
    public function testAVcSdJwtConfigurationIsPublishedWithItsResolvedContext(): void
    {
        $resolvedContext = ['https://www.w3.org/ns/credentials/v2', 'https://issuer.com/context.jsonld'];
        ...
        $this->moduleConfigMock->method('getVciCredentialConfigurationsSupported')->willReturn([
            ...
            'WithDefinition' => [
                ...
                ClaimsEnum::CredentialDefinition->value => [
                    ClaimsEnum::Type->value => ['VerifiableCredential', 'WithDefinition'],
                ],
            ],
            'NotSdJwt' => [
                ClaimsEnum::Format->value => CredentialFormatIdentifiersEnum::JwtVcJson->value,
        ...
        $resolvedFor = [];
        $this->vciContextResolverMock->method('resolve')->willReturnCallback(
            function (string $credentialConfigurationId) use (&$resolvedFor, $resolvedContext): array {
                $resolvedFor[] = $credentialConfigurationId;
        ...
        $this->assertSame($resolvedContext, $configurations['Bare'][ClaimsEnum::AtContext->value] ?? null);
        $this->assertArrayNotHasKey(ClaimsEnum::CredentialDefinition->value, $configurations['Bare']);

        $this->assertSame(
            $resolvedContext,
            $configurations['WithDefinition'][ClaimsEnum::CredentialDefinition->value][ClaimsEnum::AtContext->value]
            ?? null,
        );
        $this->assertArrayNotHasKey(ClaimsEnum::AtContext->value, $configurations['WithDefinition']);

        // The type the operator stated is still there beside it.
        $this->assertSame(
            ['VerifiableCredential', 'WithDefinition'],
            $configurations['WithDefinition'][ClaimsEnum::CredentialDefinition->value][ClaimsEnum::Type->value]
            ?? null,
        );

        // Only that format carries a JSON-LD context, so only its configurations were resolved.
        $this->assertArrayNotHasKey(ClaimsEnum::AtContext->value, $configurations['NotSdJwt']);
        $this->assertSame(['Bare', 'WithDefinition'], $resolvedFor);
    }
    ...
    public function testDescribesWhatEachConfigurationCanBeProvedAndSignedWith(): void
    {
        ...
        // The proof algorithms are what this deployment can verify, not what it signs with - so both
        // configured algorithms, once each, rather than the active key's alone.
        $this->assertSame(
            [
                OpenId4VciProofValidator::PROOF_TYPE_JWT => [
                    ClaimsEnum::ProofSigningAlgValuesSupported->value => [
                        SignatureAlgorithmEnum::ES256->value,
    ...
    /**
     * The Entity Configuration is wired to this service whether or not the deployment issues
     * credentials, and only asks for the document when it does. That arrangement holds only while
     * constructing the service reads nothing, since a deployment without Verifiable Credential settings
     * would otherwise fail to publish an Entity Configuration it has every right to.
     */
    public function testConstructingItReadsNoConfiguration(): void
    {
        ...
        $this->routesMock = $this->createMock(Routes::class);
        $this->routesMock->expects($this->never())->method($this->anything());
        $this->vciContextResolverMock = $this->createMock(VciContextResolver::class);
        $this->vciContextResolverMock->expects($this->never())->method($this->anything());
        $this->didFactoryMock = $this->createMock(DidFactory::class);
        $this->didFactoryMock->expects($this->never())->method($this->anything());
    ...
    /**
     * Asked twice, it hands back the same document rather than reading the settings again. Building
     * is not free - for a configuration which binds, it builds the DID facade, which validates the DID
     * settings and instantiates a cache adapter - and nothing about the settings changes between two
     * questions in one request. So a binding configuration, and the facade built once for the two.
     */
    public function testBuildsTheDocumentOnce(): void
    {
        ...
        $this->moduleConfigMock->method('getActiveVciSignatureKeyPair')
            ->willReturn($this->vciSignatureKeyPairBag->getFirstOrFail());
        $this->moduleConfigMock->expects($this->once())->method('getVciCredentialConfigurationsSupported')
            ->willReturn($this->credentialConfigurations());
