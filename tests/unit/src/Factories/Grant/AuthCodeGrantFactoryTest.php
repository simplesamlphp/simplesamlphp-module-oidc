...
/**
 * The factory behind the authorization code grant.
 *
 * `routing/services/services.yml` names `build` as the factory of the AuthCodeGrant service, which the
 * AuthorizationServerFactory takes in its constructor; nothing else calls it. The factory's whole job is to
 * hand the grant its thirteen collaborators and the two configured lifetimes: the authorization code lifetime
 * through the constructor, the refresh token lifetime through League's setter, over the month League's
 * constructor starts every authorization code grant with.
 *
 ...
 */
#[CoversClass(AuthCodeGrantFactory::class)]
#[UsesClass(AuthCodeGrant::class)]
...
class AuthCodeGrantFactoryTest extends TestCase
{
    ...
    protected function sut(): AuthCodeGrantFactory
    {
        return new AuthCodeGrantFactory(
    ...
    protected function propertyOf(AuthCodeGrant $grant, string $property): mixed
    {
    ...
    {
        $this->assertInstanceOf(AuthCodeGrantFactory::class, $this->sut());
    }


    /**
     * The built grant is the authorization code grant itself, not the pre-authorized code grant which extends
     * it, and every collaborator the factory was given is in its place on it.
     */
    public function testBuildsTheAuthorizationCodeGrantAroundTheFactorysCollaborators(): void
    {
        $grant = $this->sut()->build();

        $this->assertSame(AuthCodeGrant::class, $grant::class);
        $this->assertSame(GrantTypesEnum::AuthorizationCode->value, $grant->getIdentifier());
    ...
    /**
     * The authorization code lifetime is what the grant stamps on the codes it issues and the refresh token
     * lifetime what it hands the refresh token issuer, each the configured one and not the other.
     */
