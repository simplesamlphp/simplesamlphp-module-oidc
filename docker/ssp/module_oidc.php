...
/*    ModuleConfig::OPTION_AUTH_PROCESSING_FILTERS => [
    ...
    // OpenID4VCI, for the OpenID4VCI issuer test plan (conformance-tests/conformance-vci-issuer.json). Its
    // clients are registered in docker/conformance-vci.sql and the signing key pair below is generated by
    // docker/generate-dev-certs.sh. Non-registered clients stay disallowed: the plan cannot run with a
    // public client, so allowing them would only widen what the OpenID Connect plans run against.
    ...
    ModuleConfig::OPTION_VCI_CREDENTIAL_CONFIGURATIONS_SUPPORTED => [
        'ResearchAndScholarshipCredentialDcSdJwt' => [
            ...
            ClaimsEnum::CredentialMetadata->value => [
